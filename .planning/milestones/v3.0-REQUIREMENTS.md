# Requirements Archive: v3.0 Trades Page Overhaul
...
- [x] **SIDE-01**: User can see account performance panel (avg return, avg return %, win %) with cumulative chart
- [x] **SIDE-02**: User can see setups P&L breakdown (ranked list with trade count, win rate, total P&L per setup)
- [x] **SIDE-03**: User can see mistakes P&L breakdown (ranked list with count, total P&L impact per mistake type)
- [x] **SIDE-04**: Sidebar always visible on desktop (collapse removed by design); hidden on mobile with tab toggle
...
| SIDE-04 | Phase 23 | Complete (modified — always visible, no collapse) |
