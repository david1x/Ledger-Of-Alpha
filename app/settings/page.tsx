...
type Category = "account" | "accounts" | "tags" | "templates" | "display" | "chart" | "strategies" | "integrations" | "broker" | "security" | "data" | "admin-users" | "admin-settings";
...
function SettingsContent() {
  ...
  const [ibkrSyncResults, setIbkrSyncResults] = useState<Array<{ ibkrAccountId: string; ledgerAccountId: string; newCount: number; dupCount: number; errCount: number; error?: string }> | null>(null);
  ...
  useEffect(() => {
    if (activeCategory !== "accounts" || accounts.length === 0) return;
    Promise.all(
      accounts.map(async (acct) => {
        ...
        const pnl = Array.isArray(trades)
          ? trades.filter((t: any) => t.status === "closed").reduce((s: number, t: any) => s + (t.pnl ?? 0), 0)
          : 0;
      ...
      <div className="flex-1 min-w-0 space-y-6 sm:max-w-2xl min-h-[80vh]">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="number" step="0.01" value={settings.daily_loss_limit}
                    onChange={e => setSettings(s => ({ ...s, daily_loss_limit: e.target.value }))} className="flex-1 min-w-0 px-3 py-2 rounded-lg border dark:border-slate-700 border-slate-300 dark:bg-slate-800 bg-white dark:text-white text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="No limit" />
                  <select value={settings.daily_loss_limit_type}
                    onChange={e => setSettings(s => ({ ...s, daily_loss_limit_type: e.target.value }))} className="w-16 shrink-0 px-2 py-2 rounded-lg border dark:border-slate-700 border-slate-300 dark:bg-slate-800 bg-white dark:text-white text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <section className="rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white p-5 space-y-4">
              ...
              <div>
                ...
                <input type="password" value={settings.openai_api_key}
                  ...
                  placeholder="Enter your Gemini API key..." className={INPUT} />
                <p className={HINT}>
                  Get a key at <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:underline">aistudio.google.com</a>.
          ...
          <>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                <div className="rounded-2xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white p-6 w-full max-w-lg shadow-2xl space-y-4 max-h-[80vh] overflow-y-auto">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setIbkrSyncResults(null)} className="p-1.5 rounded-lg hover:dark:bg-slate-700 hover:bg-slate-100 transition-colors">
                  ...
                  <div className="space-y-3">
                    {ibkrSyncResults.map((r, i) => {
                      ...
                      return (
                        <div key={i} className="rounded-xl border dark:border-slate-700 border-slate-200 p-4 space-y-2">
                          <div className="font-medium dark:text-white text-slate-900 text-sm">{acctName}</div>
                          {r.error ? (
                            ...
                            <div className="space-y-1.5">
                              <div className="flex items-center gap-2 text-xs text-emerald-400">
                                ...
                                <span>{r.newCount} new trade{r.newCount !== 1 ? "s" : ""} imported</span>
                              </div>
                              {r.dupCount > 0 && (
                                <div className="flex items-center gap-2 text-xs dark:text-slate-400 text-slate-500">
                                  ...
                                  <span>{r.dupCount} duplicate{r.dupCount !== 1 ? "s" : ""} skipped</span>
                                </div>
                              )}
                              {r.errCount > 0 && (
                                <div className="flex items-center gap-2 text-xs text-amber-400">
                                  <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
                                  <span>{r.errCount} error{r.errCount !== 1 ? "s" : ""}</span>
                                </div>
                              )}
                            </div>
                          )}
                  ...
                  <button onClick={() => setIbkrSyncResults(null)}
            ...
            {ibkrDiscoveredAccounts.length > 0 && (
              <section className="rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white p-5 space-y-4">
                ...
                <p className="text-xs dark:text-slate-400 text-slate-500">
                  Link each IBKR sub-account to a Ledger of Alpha account. Only mapped accounts will sync trades.
                </p>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b dark:border-slate-700 border-slate-200">
                        <th className="text-left px-4 py-2.5 text-xs font-medium dark:text-slate-400 text-slate-500 uppercase tracking-wide">IBKR Account</th>
                        <th className="text-left px-4 py-2.5 text-xs font-medium dark:text-slate-400 text-slate-500 uppercase tracking-wide">Type</th>
                        <th className="text-left px-4 py-2.5 text-xs font-medium dark:text-slate-400 text-slate-500 uppercase tracking-wide">Ledger Account</th>
                        <th className="px-4 py-2.5" />
                      </tr>
                    </thead>
                    <tbody className="divide-y dark:divide-slate-700/50 divide-slate-100">
                      {ibkrDiscoveredAccounts.map(ibkrAcct => {
                        const mapping = ibkrAccountMappings.find(m => m.ibkrAccountId === ibkrAcct.id);
                        return (
                          <tr key={ibkrAcct.id}>
                            <td className="px-4 py-3 font-mono text-xs dark:text-slate-300 text-slate-700">{ibkrAcct.id}</td>
                            <td className="px-4 py-3 text-xs dark:text-slate-400 text-slate-500 capitalize">{ibkrAcct.type}</td>
                            <td className="px-4 py-3">
                              <select
                                ...
                                onChange={e => {
                                  ...
                                  setIbkrAccountMappings(prev => {
                                    const filtered = prev.filter(m => m.ibkrAccountId !== ibkrAcct.id);
                                    if (val) return [...filtered, { ibkrAccountId: ibkrAcct.id, ledgerAccountId: val }];
                                    return filtered;
                                  });
                                }}
                            ...
                            <td className="px-4 py-3">
                              {mapping?.ledgerAccountId && (
                                <button
                                  onClick={() => setIbkrAccountMappings(prev => prev.filter(m => m.ibkrAccountId !== ibkrAcct.id))}
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={async () => {
                      ...
                      await fetch("/api/settings", {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ ibkr_account_mappings: JSON.stringify(ibkrAccountMappings) }),
                      });
                  ...
                  >
                    ...
                    {ibkrMappingsSaved ? "Saved!" : ibkrMappingsSaving ? "Saving..." : "Save Mappings"}
                  </button>
                </div>
              </section>
            )}
            ...
            <section className="rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white p-5 space-y-3">
              ...
              {ibkrLastSync ? (
                <div className="space-y-2">
                  ...
                  {ibkrLastSyncStatus && (
                    <p className={clsx("text-xs font-medium", ibkrLastSyncStatus === "success" ? "text-emerald-400" : "text-red-400")}>
        ...
        {activeCategory === "accounts" && (
          <section className="rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white p-5 space-y-5">
            ...
            <p className="text-sm dark:text-slate-400 text-slate-500">Create multiple accounts (e.g. Main, Paper Trading, Futures) with separate balances and trade histories.</p>
            ...
            <div className="space-y-3">
              {accounts.map(acct => {
                ...
                return (
                  <div key={acct.id} className="p-4 rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-800/50 bg-slate-50 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          value={name}
                          onChange={(e) => setAcctEdits(prev => ({ ...prev, [acct.id]: { ...prev[acct.id], name: e.target.value } }))}
                        ...
                        />
                        {acct.is_default === 1 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 font-bold uppercase">Default</span>}
                      </div>
                      <div className="flex items-center gap-2">
                        {hasChanges && (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              setAcctEdits(prev => { const n = { ...prev }; delete n[acct.id]; return n; });
                              await refreshAccounts();
                              setAcctSaving(null);
                            }}
                            className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-bold disabled:opacity-50"
                          >
                            {acctSaving === acct.id ? <RefreshCw className="w-3 h-3 animate-spin" /> : "Save"}
                          </button>
                        )}
                        {accounts.length > 1 && (
                          <button
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      <div>
                        <label className="block text-[10px] font-medium dark:text-slate-500 text-slate-400 mb-1 uppercase tracking-wider">Starting Balance</label>
                        <input
                          type="number"
                          step="0.01"
                          value={startBal}
                          onChange={(e) => setAcctEdits(prev => ({ ...prev, [acct.id]: { ...prev[acct.id], starting_balance: parseFloat(e.target.value) || 0 } }))}
                      ...
                      <div>
                        <label className="block text-[10px] font-medium dark:text-slate-500 text-slate-400 mb-1 uppercase tracking-wider">Risk %</label>
                        <input
                          type="number"
                          step="0.1"
                          value={risk}
                          onChange={(e) => setAcctEdits(prev => ({ ...prev, [acct.id]: { ...prev[acct.id], risk_per_trade: parseFloat(e.target.value) || 0 } }))}
                          className="w-full px-2.5 py-1.5 rounded-lg border dark:border-slate-600 border-slate-300 dark:bg-slate-700 bg-white dark:text-white text-slate-900 text-sm"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-medium dark:text-slate-500 text-slate-400 mb-1 uppercase tracking-wider">Commission</label>
                        <input
                          type="number"
                          step="0.01"
                          value={comm}
                          onChange={(e) => setAcctEdits(prev => ({ ...prev, [acct.id]: { ...prev[acct.id], commission_value: parseFloat(e.target.value) || 0 } }))}
                          className="w-full px-2.5 py-1.5 rounded-lg border dark:border-slate-600 border-slate-300 dark:bg-slate-700 bg-white dark:text-white text-slate-900 text-sm"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-medium dark:text-slate-500 text-slate-400 mb-1 uppercase tracking-wider">Current Balance</label>
                        <div className={clsx("px-2.5 py-1.5 rounded-lg text-sm font-semibold", currentBal >= startBal ? "text-emerald-400" : "text-red-400")}>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && newAcctName.trim()) {
                    ...
                    }).then(() => { setNewAcctName(""); refreshAccounts(); });
                  }
                }}
