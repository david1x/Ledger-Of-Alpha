...
export default function Navbar() {
  ...
  const { accounts, activeAccountId, activeAccount, setActiveAccountId } = useAccounts();
    ...
    try {
      ...
      for (const alert of active) {
        ...
        if (alert.condition === "above") {
          ...
          triggered = pctMove >= (alert.percent_value ?? 0);
        } else if (alert.condition === "percent_move" && alert.anchor_price) {
          const pctMove = Math.abs(((price - alert.anchor_price) / alert.anchor_price) * 100);
          triggered = pctMove >= (alert.percent_value ?? 0);
        }
        if (triggered) {
  ...
  const nav = (
    <>
        ...
        <div className="flex flex-col h-full w-full">
          <div className={clsx("h-16 flex items-center shrink-0 border-b dark:border-slate-800/50 border-slate-100/50 overflow-hidden", showLabels ? "px-5" : "justify-center px-0")}>
            <Link href="/" className="flex items-center gap-2.5 font-bold tracking-tight">
              ...
              {showLabels && <div className="flex items-center gap-1"><span className="text-emerald-400">Ledger</span><span className="dark:text-white text-slate-900">Of Alpha</span></div>}
            </Link>
          </div>

          {/* Account Switcher */}
          {accounts.length > 0 && (
            <div className="px-2 py-2 border-b dark:border-slate-800/50 border-slate-100/50" ref={acctDropRef}>
                ...
                >
                  ...
                  {showLabels && (
                    <>
                      ...
                      <ChevronDown className={clsx("w-3.5 h-3.5 dark:text-slate-500 text-slate-400 transition-transform", acctDropOpen && "rotate-180")} />
                    </>
                  )}
                </button>
                {acctDropOpen && (
                  <div className={clsx(
                    "absolute z-50 dark:bg-slate-900 bg-white border dark:border-slate-700 border-slate-200 rounded-xl shadow-2xl overflow-hidden",
                    showLabels ? "left-0 right-0 top-full mt-1" : "left-14 top-0 w-52"
                  )}>
                    <div className="py-1 max-h-64 overflow-y-auto">
                      <button
                        onClick={() => { setActiveAccountId(null); setAcctDropOpen(false); }}
                        className={clsx(
                      ...
                      {accounts.map(acct => (
                        <button
                          key={acct.id}
                          onClick={() => { setActiveAccountId(acct.id); setAcctDropOpen(false); }}
                          className={clsx(
                            "flex items-center gap-2 w-full px-3 py-2 text-xs font-medium transition-colors",
                        ...
                        >
                          ...
                          {acct.is_default === 1 && <span className="text-[9px] dark:text-slate-500 text-slate-400 ml-auto">default</span>}
                        </button>
                      ))}
                    </div>
                    <div className="border-t dark:border-slate-800 border-slate-100 p-1">
                      <Link
                        href="/settings?tab=accounts"
                        onClick={() => { setAcctDropOpen(false); setMobileOpen(false); }}
                        className="flex items-center gap-2 w-full px-3 py-2 text-xs font-medium dark:text-slate-400 text-slate-500 hover:dark:bg-slate-800 hover:bg-slate-50 rounded-lg transition-colors"
                      >
                ...
                <button onClick={() => setMenuOpen(v => !v)}
                  ...
                  {showLabels && (
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <p className="text-[10px] dark:text-slate-500 text-slate-400 truncate uppercase tracking-wider">{me.guest ? "Guest Mode" : me.isAdmin ? "Administrator" : "Trader"}</p>
                    </div>
                  )}
                </button>
                {menuOpen && (
                  <div className={clsx("absolute bottom-full left-2 right-2 mb-2 dark:bg-slate-900 bg-white border dark:border-slate-700 border-slate-200 rounded-xl shadow-2xl overflow-hidden z-50", !showLabels && "w-48 left-14 bottom-2")}>
                    ...
                    <div className="p-1">
                      {me?.isAdmin && <Link href="/admin" onClick={() => setMenuOpen(false)} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-emerald-400 hover:dark:bg-slate-800 hover:bg-slate-50 rounded-lg transition-colors"><ShieldCheck className="w-4 h-4" /> Admin Panel</Link>}
                      {!me.guest && <Link href="/settings" onClick={() => setMenuOpen(false)} className="flex items-center gap-2 w-full px-3 py-2 text-sm dark:text-slate-300 text-slate-700 hover:dark:bg-slate-800 hover:bg-slate-50 rounded-lg transition-colors"><SettingsIcon className="w-4 h-4" /> Account Settings</Link>}
                      <button onClick={handleSignOut} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-red-400 hover:dark:bg-slate-800 hover:bg-slate-50 rounded-lg transition-colors"><LogOut className="w-4 h-4" /> {me.guest ? "Exit Guest Mode" : "Sign Out"}</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      <AlertModal open={showAlertModal} onClose={() => { setShowAlertModal(false); setEditAlert(null); }} onSaved={loadAlerts} editAlert={editAlert} />
    </>
  );
      ...
      {toasts.map((t) => (
        <div key={`${t.id}-${t.ts}`} className="pointer-events-auto rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white shadow-2xl p-4" style={{ animation: "slideInRight 0.3s ease-out" }}>
          <div className="flex items-start gap-3">
            ...
            <button onClick={() => setToasts(prev => prev.filter(x => x.ts !== t.ts))} className="p-0.5 rounded hover:dark:bg-slate-800 hover:bg-slate-100 transition-colors shrink-0"><X className="w-3.5 h-3.5 dark:text-slate-500 text-slate-400" /></button>
          </div>
        </div>
      ))}
    </div>,
