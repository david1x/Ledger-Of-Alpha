...
import { ShieldCheck, ShieldOff, Copy, Eye, EyeOff, KeyRound, Save, CheckCircle } from "lucide-react";
...
export default function SecurityTab() {
  ...
  const changePassword = async () => {
    ...
    if (!currentPw || !newPw || !confirmPw) {
      setPwMsg({ text: "All fields are required.", type: "err" });
      return;
    }
    if (newPw !== confirmPw) {
      setPwMsg({ text: "New passwords do not match.", type: "err" });
      return;
    }
    if (newPw.length < 8) {
      setPwMsg({ text: "New password must be at least 8 characters.", type: "err" });
      return;
    }
    setPwSaving(true);
    ...
    if (res.ok) {
      ...
      setPwMsg({ text: data.error, type: "err" });
    }
  };
  ...
  return (
    ...
    <section className="rounded-xl border dark:border-slate-700 border-slate-200 dark:bg-slate-900 bg-white p-5 space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-2.5 top-1/2 -translate-y-1/2 dark:text-slate-400 text-slate-500"
            >
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <Save className="w-4 h-4" /> {pwSaving ? "Saving..." : "Change Password"}
        </button>
        {pwMsg?.type === "ok" && (
